/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PerformanceReviewWhereInput } from "./PerformanceReviewWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { PerformanceReviewOrderByInput } from "./PerformanceReviewOrderByInput";

@ArgsType()
class PerformanceReviewFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => PerformanceReviewWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => PerformanceReviewWhereInput, { nullable: true })
  @Type(() => PerformanceReviewWhereInput)
  where?: PerformanceReviewWhereInput;

  @ApiProperty({
    required: false,
    type: [PerformanceReviewOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [PerformanceReviewOrderByInput], { nullable: true })
  @Type(() => PerformanceReviewOrderByInput)
  orderBy?: Array<PerformanceReviewOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { PerformanceReviewFindManyArgs as PerformanceReviewFindManyArgs };
